...
module.exports = function(opts, cards, git, parentCallback){
    ...
    collectionManager.bindListeners = function(opts, socket){
        ...
        socket.on("collection:checkpoint:resolve", function(resolveWithCheckpoint){
            if(resolveWithCheckpoint){
                if (opts.debug) console.log("Resolving defferal with checkpoint.");
                collectionManager.collection = collectionManager.lastCheckpoint;
            }
            else{
                if (opts.debug) console.log("Resolving defferal with collection.");
            }
            collectionManager.deferedCheckpointResolution = false;
        ...
        socket.on("collection:import", function(cards){
            if(opts.debug) console.log("Importing Bulk Cards");
            var cards = collectionManager._inflate(cards.split("\n"));
                ...
                fs.writeFile(opts.data + "/collection.mbc", out, function(err){
                    if(err){
                        ...
                        if(fs.existsSync(opts.data + "/collection.checkpoint.mbc")){
                            ...
                            fs.unlinkSync(opts.data + "/collection_data.checkpoint.json");
    ...
    collectionManager.remove = function(card){
        ...
        for(var i=0; i<collectionManager.collection.length; i++){
            var collectionCard = collectionManager.collection[i];
            if(collectionCard.name === card.name){
                collectionManager.collection.splice(i, 1);
                break;
            }
        };
    };
    ...
    collectionManager._loadCheckpoint = function(callback){
        ...
        jsonfile.readFile(opts.data + "/collection_data.checkpoint.json", function(err, obj){
            ...
            lineReader.on('close', function(){
                ...
                collectionManager.lastCheckpoint = collectionManager._inflate(lines);
                if(typeof callback === 'function'){
                    callback(collectionManager.lastCheckpoint);
                }
                else{
                    return collectionManager.lastCheckpoint;
                }
            });
        });
    }
    ...
    function startTimers(){
        collectionManager.checkpointTimer = setInterval(function(){
            if(!collectionManager.deferredCheckpointResolution){
                collectionManager.checkpoint();
            }
